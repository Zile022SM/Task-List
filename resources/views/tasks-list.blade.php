        ...
        @if(count($tasks) > 0)
            <div class="justify-start">
                <ol>
                    ...
                    <li class="flex items-center gap-2 {{ $task->id === session('updated_task') ? 'bg-orange-300 py-2' : '' }}">
                        <span class="text-orange-500">{{ $task->id }}</span> - 
                        <a href="{{ route('single-task', $task->id) }}" class="text-blue-500 hover:underline">{{ $task->title }}</a>
